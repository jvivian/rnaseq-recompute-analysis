...
class Analysis(object):
    ...
    def __init__(self, tissue_df, cores, gene_map, gencode_path='/mnt/gencode.v23.annotation.gtf'):
        ...
        global edger_script
        edger_script = os.path.join(self.tissue_dir, 'edgeR-pairwise-DE.R')
        self.edger_script = edger_script

        tcga_cols = filter(lambda x: 'TCGA' in x, self.df.columns)
        self.tcga_cols = [x.replace('-', '.') for x in tcga_cols]
    ...
    def run_pairwise_edger(self):
        ...
        # _run_edger(self.tcga_cols[0])
        pool.map(_run_edger, self.tcga_cols)

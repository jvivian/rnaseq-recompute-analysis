...
class Analysis(object):
    ...
    def run_pairwise_edger(self):
        ...
        pool.map(functools.partial(_run_edger, edger_script=self.edger_script), self.tcga_names)
...
def _run_edger(sample, edger_script):
    """
    Function used in pool.map to run Rscript
    Use functools.partial or a lambda to pass the static edger_script variable

    :param str sample: TCGA sample
    :param str edger_script: Path to the edger_script generated by _generate_edger_script
    """
    print 'Running sample: ' + sample
    try:
        subprocess.check_call(['Rscript', edger_script, sample])
